...
NSFW_SETTINGS = {}  # chat_id: {"silent": bool, "punish": bool}
USER_WARNINGS = {}
USER_LAST_SCAN = {}
...
async def add_amthy(client, message):
    ...
    if not message.reply_to_message:
        return await message.reply_text("Reply to user to add as sudo.")
...
async def nsfw_silent(client, message):
    ...
    if message.command[1].lower() == "on":
        ...
        await message.reply_text("✅ NSFW Silent Mode Enabled.")
    else:
        NSFW_SETTINGS[chat_id]["silent"] = False
        await message.reply_text("❌ NSFW Silent Mode Disabled.")
...
@app.on_message(filters.command("nsfwcommands"))
...
async def nsfw_handler(client, message):
    ...
    # Auto delete warning after 60 sec
    if warn_msg:
        asyncio.create_task(delete_later(warn_msg, 60))

    if is_admin:
        return  # Admins pe smart punish apply nahi hoga
